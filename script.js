...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderStoresOnPage(stores) {
        ...
        if (stores.length === 0) {
            ...
            if (vendedorRegStore) vendedorRegStore.innerHTML = '<option value="">Nenhuma ótica licenciada cadastrada</option>';
        ...
        stores.forEach(store => {
            ...
            if (vendedorRegStore) {
                const vendOpt = document.createElement('option');
                vendOpt.value = store.nome;
                vendOpt.textContent = store.nome;
                vendedorRegStore.appendChild(vendOpt);
            }
...
Apresente esse cupom na loja para garantir o seu benefício!`;
    ...
    // -------------------------------------------------------------
    // Plataforma de Prêmios para Vendedores (Balconistas)
    // -------------------------------------------------------------
    ...
    async function loadRewardsConfig() {
        ...
        if (url && key) {
            try {
                const cleanUrl = url.replace(/\/$/, "").replace(/\/rest\/v1$/, "");
                const endpoint = `${cleanUrl}/rest/v1/premios_config_personality`;
                const response = await fetch(endpoint, {
                    method: 'GET',
                    headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
                });
            ...
            if (localConfig) {
                ...
                localStorage.setItem('personality_premios_config', JSON.stringify(currentRewardsConfig));
            }
        }
    ...
    function populateProductSelects() {
        ...
        if (saleLensFamily) {
            ...
            currentRewardsConfig.filter(c => c.categoria === 'lente').forEach(item => {
                const opt = document.createElement('option');
                opt.value = item.nome;
                opt.textContent = `${item.nome} (${item.pontos} Pts / R$ ${item.valor.toFixed(2)})`;
    ...
    // WhatsApp Masking para o Vendedor
    applyPhoneMask(vendedorRegWhatsapp);
        ...
        vendedorRegisterForm.addEventListener('submit', async (e) => {
            ...
            if (url && key) {
                try {
                    ...
                    const checkEndpoint = `${cleanUrl}/rest/v1/vendedores_personality?usuario=eq.${encodeURIComponent(userVal)}`;
                    ...
                    // Faz o insert
                    const insertEndpoint = `${cleanUrl}/rest/v1/vendedores_personality`;
                    ...
                    alert('Cadastro de vendedor realizado com sucesso! Faça login para começar.');
                    ...
                    if (error.message !== 'Usuário já cadastrado.') {
                        ...
                        alert('Cadastro salvo localmente devido a indisponibilidade do banco.');
                        vendedorRegisterForm.reset();
                        vendedorRegisterCard.style.display = 'none';
                        vendedorLoginCard.style.display = 'block';
                    }
    ...
    if (vendedorLoginForm) {
        vendedorLoginForm.addEventListener('submit', async (e) => {
            ...
            if (url && key) {
                try {
                    const cleanUrl = url.replace(/\/$/, "").replace(/\/rest\/v1$/, "");
                    const endpoint = `${cleanUrl}/rest/v1/vendedores_personality?usuario=eq.${encodeURIComponent(userVal)}&senha=eq.${encodeURIComponent(passVal)}&limit=1`;
    ...
    if (vendedorSubmitSaleForm) {
        vendedorSubmitSaleForm.addEventListener('submit', async (e) => {
            ...
            if (url && key) {
                try {
                    const cleanUrl = url.replace(/\/$/, "").replace(/\/rest\/v1$/, "");
                    const endpoint = `${cleanUrl}/rest/v1/premios_lancados_personality`;
    ...
    async function loadVendedorExtrato(vendedorId) {
        ...
        if (url && key) {
            ...
            try {
                const cleanUrl = url.replace(/\/$/, "").replace(/\/rest\/v1$/, "");
                const endpoint = `${cleanUrl}/rest/v1/premios_lancados_personality?vendedor_id=eq.${encodeURIComponent(vendedorId)}&order=created_at.desc`;
    ...
    function renderVendedorExtrato(sales) {
        ...
        // Atualiza cards de estatísticas no painel do vendedor
    ...
    // Carrega a configuração inicial de prêmios e verifica a sessão
    loadRewardsConfig();
    checkVendedorSession();
